...
import io.arenadata.dtm.query.execution.plugin.adqm.dml.service.upserts.DestinationUpsertSelectHandler;
...
@Service("adqmUpsertSelectService")
public class AdqmUpsertSelectService implements UpsertSelectService {
    private final Map<SourceType, DestinationUpsertSelectHandler> handlers;

    public AdqmUpsertSelectService(List<DestinationUpsertSelectHandler> handlers) {
    ...
    public Future<Void> execute(UpsertSelectRequest request) {
        ...
        for (SourceType sourceType : request.getEntity().getDestination()) {
            ...
            if (handler == null) {
                futures.add(Future.failedFuture(new DtmException(String.format("Upsert select from [ADQM] to [%s] is not implemented", sourceType))));
                break;
            }
